...
# ただし相手チームスロット2はIssue #57対応により∞判定自体を行わないため、
# 70/71/12でこのスロットだけ期待値どおりにならないことがある(vs_rank.pyの
# モジュールdocstring参照、既知のトレードオフ)。全体の正答率で判断する
# アサーション(閾値0.85)なので個別の不一致自体は許容している
