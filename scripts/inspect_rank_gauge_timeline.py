...
from nss_tracker.detection.rank_ocr import GAUGE_ROI_ENLARGED, RANK_NUMBER_ROI_ENLARGED, RANK_ROI, read_precise_rank
...
def main(video_name: str, start: int, end: int, ocr_every: int = 5) -> None:
    ...
    for idx, frame in enumerate(read_frames(path)):
        ...
        if idx % ocr_every == 0:
            ...
            result = read_precise_rank(frame, GAUGE_ROI_ENLARGED, RANK_NUMBER_ROI_ENLARGED)
            ocr = f" ocr={result}"
