...
S/A帯での昇格/降格映像では未検証だった(参照fixture(70/71)は静止画1枚ずつのみで
前後比較ができないため)。ただしIssue #100対応時にユーザーからゲーム内の実際の
仕様(∞は0〜49、Sは0〜9、Aは最大29で下限不明。各帯とも帯内の上限に到達すると
次の帯へ自動的に昇格する)の説明を受けており、この仕様上はS/A帯も∞と同じ向き
(大きいほど良い)であることが確定している。この仕様に基づき、
`web/server.py`の対戦相手ランク比較(Issue #100)では∞/S/Aを跨いだ換算スケール
(∞はそのまま、Sは値−10、Aは値−40)で合算している。とはいえ実際の昇格/降格映像
での映像ベースの検証はまだ行っていないため、大小比較による昇格/降格判定を
vs_rank.py側で行う場合は引き続き別途の検証が必要(現時点でread_vs_screen_ranksの
呼び出し元はスナップショットとして保存するのみで、前後比較はしていない)。
...
Issue #144対応(2026-07-31): MINE_ICON_XYWHをユーザーが再実測し、各スロットの
高さをやや拡張した(スロット0: 高さ33→38、スロット1: 26→34、スロット2: 22→26、
スロット3: 高さ16→22かつy1を629→626に上へ拡張)。幅・x1・y1(スロット0-2)は
