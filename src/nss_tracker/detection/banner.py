...
Issue #431対応(2026-09-22、LOSE_HUE_RANGE/LOSE_SAT_RANGE再較正): 上記の計測で
実測が揃ったため再較正した。2026-09-21の配信は1セッションの中に専用部屋11試合
(23:07〜00:01)と野良9試合(00:11〜01:06)が同居しており、同じプロセス・同じ
Virtual Camera・同じ日という条件で両者を直接比較できた。専用部屋側は11試合中
**勝ち4試合しか記録されず、負け6試合と引き分け1試合が丸ごと消えた**(Issue #423の
再現)一方、野良側は9試合すべて記録できていた。

保存した実フレーム(banner_debug_frames.py)を測り直すと、負けバナーが止まっている
間の値は**専用部屋・野良を問わず完全に同一**だった:

    フェーズ1(明るい状態)      H=104.61 S=34.95 V=84.75 hue_std=1.91 -> 判定なし
    フェーズ2(暗転演出が乗る)  H= 99.83 S=42.08 V=54.05 hue_std=0.99 -> "lose"

違うのはフェーズ2があるかどうかだけだった。野良はバナー表示の約2秒後にランク変動
演出で画面全体が暗くなりはじめ、ROI平均がたまたま閾値の内側へ入る。専用部屋は
ランクを賭けないためこの演出が無く、フェーズ1が6.0〜6.5秒そのまま静止して
内側に入る瞬間が一度も来ない。つまり野良で検知できていたのは偶然で、専用部屋には
「たまたま助かる瞬間」が構造的に存在しなかった。Issue #423(専用部屋で消える)と
Issue #431(野良でも1.6〜2.4秒遅れる)は同一の根本原因だったことになる。

ローカル録画とVirtual Cameraの差の正体も特定できた。同じ瞬間をOBSローカル録画
(mkv)から測るとH=101.50 S=40.73 V=84.31(Issue #423の報告値がそのまま再現)で、
BANNER_ROISの生のBGRまで降りると差はわずか**2階調**だった:

    mkvローカル録画      B=85 G=79 R=71  -> H=103 S=42 V=85 (閾値の内側)
    Virtual Camera       B=85 G=79 R=73  -> H=105 S=36 V=85 (色相が上限外)

負けバナーの帯は**ほぼ無彩色の暗いグレー**(最大-最小のクロマ幅が14/255しかない)
のため、Rが1階調動くだけでOpenCVのHが約1.0〜1.5・Sが約3も振れる。ローカル録画と
Virtual Cameraの間のYUV量子化差(2階調)だけで、残っていたマージン(色相1.5・
彩度5.7)を丸ごと食い潰していた。閾値が際どかったのではなく、**色相・彩度という
指標自体がこの色に対しては不安定**というのが本質で、Issue #373(引き分け文字の
明度)と同根である。Vだけは両者で一致しており安定している。

再較正の根拠にした実測(6セッション・負け31試合、logs/の_log_banner_stats出力):

    H 101.95 .. 104.61 / S 34.44 .. 38.29 / V 75.05 .. 87.53 / hue_std 1.91 .. 4.49

これに対しLOSE_HUE_RANGEの上限を103→108(実測最大+3.4)、LOSE_SAT_RANGEの下限を
35→30(実測最小-4.4)へ広げた。上記のとおり指標自体が1階調の量子化差に弱いため、
マージンは広めに取っている(BGR(85,79,75)相当=Rがもう2階調ずれても内側に留まる)。

誤検知が増えないことを3系統で確認済み:

- fixtures/screenshots全37枚: 判定が変わった枚数0
- fixtures/videos全23本(約41,000フレーム): 既知の誤検知動画
  (25_inplay_false_positive_win_blue_teal_canopy、22・23・24・32番)はいずれも変化なし。
  新たに"lose"になるのは本物のlose動画(29・31・33・39・43番)のみ。勝ち動画
  (28・30番)で1フレームだけ変わる箇所があるが、前後が全てNoneの孤立フレーム
  (0.02秒)でデバウンス(1〜2秒)を満たせず確定には至らない
- 実配信ログ全39,656行: 新たに"lose"になる98行はすべて結果バナー区間内(試合終了
  +0〜25秒)で、うち94行が本物のフェーズ1クラスタ。**通常プレイ・マッチング画面・
  VS画面にあたる30,251行では新たな"lose"判定が0件**(Issue #45/#67で誤検知していた
  画面はこの30,251行に含まれる。結果バナーを確定できなかった試合では
  _log_banner_stats()が次の試合のプレイ中もそのまま出力を続けていたため、
  意図せず通常プレイ側のサンプルも大量に集まっていた)

効果の見積もり: 負け31試合で「"lose"と判定できる最初の瞬間」が中央2.03秒・最大
5.93秒早まる。専用部屋で消えていた試合は、保存フレームでの再判定で1試合目が
"lose"6秒連続、4試合目が"draw"7秒連続、8試合目が"lose"3秒連続となり確定できる。
"""
...
# Issue #431/#423: OBS Virtual Camera経由の負けバナーは、止まっている間ずっと
# 色相の上限・彩度の下限のすぐ外側に張り付いている。6セッション・負け31試合の実測
# (H101.95〜104.61 / S34.44〜38.29 / V75.05〜87.53 / hue_std1.91〜4.49)に
# マージンを取り、色相の上限を103→108(実測最大+3.4)・彩度の下限を35→30
# (実測最小-4.4)に再較正した。帯の色がほぼ無彩色でH/Sが1階調の量子化差に
# 非常に弱いことが分かったため、マージンは広めに取っている
# (詳細はモジュールdocstringのIssue #431の節参照)
LOSE_HUE_RANGE = get_detection_value("banner", "LOSE_HUE_RANGE", (87, 108))
LOSE_SAT_RANGE = get_detection_value("banner", "LOSE_SAT_RANGE", (30, 65))
