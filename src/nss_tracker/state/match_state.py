...
Issue #67: ゴール演出中(背景に流れる斜めの「SOCCER」トランジション帯)が
デバウンスをすり抜けるほど長くBANNER_ROIの色条件に一致し続け、結果バナーの
誤検知(試合が2つに分割される)が実データで発生した。そのため_watch_for_banner()は
is_goal_event(frame)がTrueの間、classify_bannerの結果によらずbanner_streakを
強制的にリセットする(ゴール演出中は結果バナー判定自体を行わない)。ゴール演出の
検知はis_goal_event()の色チェック自体は毎フレーム軽量に呼べるため、_check_for_goal
と重複して呼んでも問題ない。なお、誤検知がゴール演出のポップアップ表示期間そのものを
超えて(例えばゴール後のキックオフカウントダウン中まで)続く場合は、この方式だけでは
防げない可能性がある。その場合は演出終了後も一定フレーム分抑制を延長するクールダウンの
追加を検討すること(実データでの検証待ち、Issue #67参照)。
...
class MatchStateMachine:
    ...
    def _watch_for_banner(self, frame: np.ndarray) -> Optional[MatchResult]:
        if is_goal_event(frame):
            # ゴール演出中は背景に流れる斜めの「SOCCER」トランジション帯がBANNER_ROIに
            # 写り込み、classify_bannerが結果バナーと誤判定することがある(Issue #67)。
            # is_goal_eventは色ベースの軽量チェックのため、_check_for_goalと重複しても
            # 毎フレーム呼んで問題ない。ゴール演出が終わればバナー判定を通常どおり再開する
