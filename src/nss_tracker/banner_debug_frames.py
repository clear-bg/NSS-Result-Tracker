...
**この仕組みは閾値の再較正が終わるまでの調査用。** Issue #431で再較正そのものは
完了したが、実配信での確認(野良で判定遅れが無くなったこと・専用部屋で負け試合が
消えないこと)が済むまでは残しておき、Issue #423/#431のクローズ時に
`state/match_state.py`の`_log_banner_stats()`とあわせて削除する
(ユーザーとの相談で決定)。`MAX_FILES`も同じタイミングで元の240へ戻す。
"""
...
# 保存先に残す最大ファイル数。1枚0.5MB程度・1試合あたり最大12枚。
# Issue #431: 再較正の実機確認をしている間に、確認したい試合の静止画が
# 保持件数の上限で消えてしまうのを避けるため、240枚(=20試合分、約130MB)から
# 600枚(=50試合分、約330MB)へ一時的に増やしている。Issue #423/#431の実機確認が
# 済んだら、この仕組み自体(モジュールdocstring末尾参照)とあわせて元に戻す
MAX_FILES = 600
