...
ランク数値(∞帯の数字のみ)を1行=1スロット(最大1試合あたり8行)で保存する。
goalsテーブルと同じくmatches.idをmatch_idとして参照する。名前を持たない
(ランクの数値のみを記録する、個人の識別情報は持たない設計。CLAUDE.md・Issue #39
参照)ため、goalsのような許可リストによるフィルタリングは行わず、読み取れな
かったスロット(文字階級バッジ・ランク非表示の試合を含む)もrank_tierをNULLとした
まま全スロット分そのまま保存する(detection.vs_rank側もポリシーを持たず見えた
ものをそのまま報告する設計と対応させている)。
"""
...
CREATE TABLE IF NOT EXISTS vs_slot_ranks (
    ...
    rank_tier INTEGER,              -- 読み取れなかった場合(文字階級・非表示含む)NULL
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL
);
"""
...
def save_vs_slot_ranks(
    conn: sqlite3.Connection,
    match_id: int,
    mine_ranks: list[Optional[int]],
    opponent_ranks: list[Optional[int]],
) -> list[int]:
    """VS画面確定時点の両チーム最大4人分のランク数値をvs_slot_ranksテーブルに保存する。

    mine_ranks/opponent_ranksはそれぞれスロット0(カメラに最も近い位置、
    自チーム側は自分自身)〜3(最も奥)の順(detection.vs_rank.read_vs_screen_ranks
    の戻り値と対応させること)。読み取れなかったスロット(None)もrank_tierが
    NULLの行としてそのまま保存する(goalsと異なり許可リストによるフィルタリングは
    行わない、モジュールdocstring参照)。挿入した各レコードのidをリストで返す。
    """
    now = now_jst().isoformat()
    inserted_ids = []
    for side, ranks in (("mine", mine_ranks), ("opponent", opponent_ranks)):
        for slot_index, rank_tier in enumerate(ranks):
            cursor = conn.execute(
                "INSERT INTO vs_slot_ranks (match_id, side, slot_index, rank_tier, created_at, updated_at) "
                "VALUES (?, ?, ?, ?, ?, ?)",
                (match_id, side, slot_index, rank_tier, now, now),
            )
